...
- **Voice VLAN now** (already just purpose:voice), **wireless controller
  deferred** to a follow-up. → **Wireless controller now DONE** — see
  "campus wireless controller" below.
...
## Sketch / diagram upload → rebuilt design — BUILT (end to end), needs a real-file shakedown

**FOUNDER DECISIONS (2026-08), which settled the two open questions below:**
1. **Formats:** jpeg, png, pdf, doc, docx, Visio "and other popular formats".
2. **Correction policy:** "only produce design with recommended corrections
   that will allow the engineer to decide" → the design reflects **what was
   drawn**; every improvement is a **recommendation**, never silently applied.
   This is the same discipline as config-import findings, so it reuses the
   same type, the same DB column and the same UI.

### What shipped

**`packages/doc-ingest` (new)** — turns an upload into material a model can
read, by one of two paths:
- **Structured (parsed, not looked at):** `.vsdx`/`.vsdm` is XML recording
  shapes, shape data and connections, so it's read deterministically —
  `visio.ts` extracts shapes, their Visio *shape data* (our own exporter writes
  role/vendorHint/hostname there, so an exported diagram round-trips with its
  semantics) and the connector→endpoint pairs. `.drawio`/`.svg`/`.xml` are
  passed through as diagram XML.
- **Visual:** png/jpeg/gif/webp and pdf go to the model as image/document
  blocks. A `.docx` is unzipped for its pasted screenshots **and** its text
  (the requirements are often in the prose, not the picture).
- **Refused by name, never half-read:** legacy `.doc` and `.vsd` (binary
  formats needing a converter we can't run on Vercel) with instructions to save
  as .docx/.vsdx or export a PDF; oversized images with advice to re-save.
  Format detection falls back to **magic bytes** when an extension lies.
- **Tests: 19.** The headline is a REAL `.vsdx` generated by this repo's own
  `services/vsdx` exporter from G1, checked in at
  `test/fixtures/g1-branch-office.vsdx` — same "we own both halves" round trip
  as config-parse. It recovers all 5 devices, their embedded roles, and all 5
  links. (Regeneration command is in the test's header comment.)

**Sketch reading (`packages/llm-extraction/src/sketch{,-prompt}.ts`)** —
`extractDesignFromDiagram(assets)` → `{ params, confidence, observations,
recommendations }`. The tool schema **extends `designParamsSchema`** so the
extra reading fields strip off and the rest goes straight to the existing
`composeDesign` dispatcher: a sketch and a paragraph of prose converge on the
same engine, same validation, same golden path. The system prompt's load-bearing
rule is "reproduce, then recommend" — one router drawn means one router
emitted; the missing firewall becomes a recommendation, not a device.
`confidence` is required, and **low confidence routes to the existing clarify
path** rather than returning a convincing wrong topology.

**`Finding` moved to `packages/schema`** (`zod/finding.ts`) so config-import
findings and sketch recommendations are one contract — one persisted format,
one UI. `config-parse` re-exports it, so existing imports still work.

**Web (`apps/web`)** — deliberately **two steps, because a diagram is lossy**:
- `POST /api/sketch/read` — multipart upload → ingest → read. **Saves
  nothing.** This is the metered step (it's the one that calls Claude); emits a
  `sketch_read` PostHog event with confidence + recommendation codes.
- `/projects/sketch` — upload page, then a **review screen** showing what was
  read, what was assumed, the recommended changes (clearly "your call, not
  applied") and how each file was read. Nothing exists until the engineer
  confirms.
- `POST /api/sketch/create` — re-validates the (client-returned, therefore
  untrusted) reading against the same schemas, composes, saves with
  `source_kind = 'sketch-import'`, recommendations in `findings`.
- Project page: a sketch project gets a **Recommendations** tab (FindingsList
  with sketch wording) and **no** "Uploaded config" tab — there's no
  configuration to read back. `source_configs` holds a file *manifest* for a
  sketch import, and the page never hands that to the config viewer.
- **`0006_sketch_import.sql` — RUN IN THE SUPABASE SQL EDITOR before deploying.**
  It only widens the `source_kind` check constraint to allow `'sketch-import'`.

### Still to do
1. **Shakedown with real files** — every format path is unit-tested, but only
   the .vsdx path has been exercised against a genuine file. Worth uploading a
   real whiteboard photo, a real Visio export and a real .docx before showing
   testers.
2. **Store the original upload.** Diagram bytes belong in Supabase storage
   (private bucket, owner-scoped); today only the file manifest is kept, so a
   sketch project can't show you the picture it came from. This is the one
   piece of the original build order not yet done.
3. **Editable review step.** The engineer can confirm or go back, but can't yet
   correct a misread device count in place — they re-upload or fix the design
   after. `/api/sketch/create` already re-validates whatever it's given, so the
   API side is ready for it.
4. **.doc / .vsd** stay refused. Supporting them needs a converter
   (LibreOffice) that can't run on Vercel — would need a service. Flagging
   rather than deciding: say the word if testers actually hit this.
...
## Campus wireless controller — DONE

Closes the follow-up deferred when campus three-tier shipped. `wireless-controller`
already existed as a device role (with a diagram icon, colour and role rank), so
this was a params + composer change only.

- `packages/schema`: optional campus-only `wirelessController {present, vendorHint}`.
- `packages/design-engine/src/compose/campus.ts`: places `wlc-01`, trunks it to
  **both** distribution switches, gives it a management address. Deliberately
  **not** an L3 device — no loopback, no /31, no SVIs, not an OSPF speaker —
  because a WLC is a service-block appliance, not a routing peer. (It falls out
  for free: `wireless-controller` isn't in `L3_CAPABLE_ROLES`, so the link is an
  L2 trunk.)
- **Ordering matters:** the controller's trunks are appended AFTER the access
  trunks, because `assignInterfaces` anchors a distribution switch's SVIs to the
  first trunk it's handed and those belong on the access side. The unchanged G2
  golden snapshot is the proof.
- **Access points are not modeled** — a floor-plan/RF-survey concern, not a
  topology one. The composer records that, and the placement, as assumptions so
  the engineer can challenge both; the extractor routes AP counts into a
  "Not yet modeled:" assumption.
- `test/golden/compose-g2-wireless.test.ts` (7 tests), plus a prompt/few-shot
  update so "wireless controller" reaches the new field instead of being dropped.

**FOUNDER REVIEW (networking call I made):** a WLC trunked to the distribution
pair with its management interface in the management network is the textbook
service-block placement, but say the word if you'd rather it hung off the core,
or wanted a dedicated wireless-management VLAN rather than reusing the mgmt net.
